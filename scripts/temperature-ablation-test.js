...
async function runTest(testCase, temperature, runIdx) {
  ...
  try {
    ...
    return { temperature, run: runIdx, ...extracted, reasoning: result.text?.slice(0, 200) };
  } catch (e) {
    return { temperature, run: runIdx, outcome: 'ERROR', detail: e.message };
  }
}
...
async function main() {
  ...
  console.log('=== Temperature Ablation Test ===\n');
  ...
  for (const tc of testCases) {
    ...
    for (const temp of temperatures) {
      ...
      for (let i = 0; i < runsPerTemp; i++) {
        ...
        console.log(`${r.outcome}${r.detail?.sizePercent ? ` (size: ${r.detail.sizePercent})` : ''}${r.detail?.reason ? ` — ${r.detail.reason.slice(0, 80)}` : ''}`);
  ...
  for (const [tcName, tempResults] of Object.entries(results)) {
    console.log(`\n${tcName}:`);
    console.log('| Temperature | Execute | Reject | Hold | Error | Notes |');
    console.log('|-------------|---------|--------|------|-------|-------|');
    for (const temp of temperatures) {
      const runs = tempResults[temp];
      const counts = { EXECUTE: 0, REJECT: 0, HOLD: 0, ERROR: 0, NO_TOOL_CALL: 0 };
      runs.forEach(r => counts[r.outcome]++);
      const notes = runs.map(r => r.detail?.reason?.slice(0, 40) || r.detail?.slice?.(0, 40) || '').filter(Boolean).join('; ');
      console.log(`| ${temp}         | ${counts.EXECUTE}/${runsPerTemp}     | ${counts.REJECT}/${runsPerTemp}    | ${counts.HOLD}/${runsPerTemp}  | ${counts.ERROR}/${runsPerTemp}   | ${notes.slice(0, 60)} |`);
    }
  }
